...
for filePAQ in $1/PAQ/*; do
	...
	resultado_chiLambda=`../../GeneracionTexto/metricas/chiLambda.py`
	...
	echo $filePAQ "," $resultado_chi "," $resultado_chiLambda "," $resultado_total "," $resultado_jaccard  >> ResultadosMetricas.txt
done
...
for fileRNN in $1/RedNeuronal/*; do
	...
	resultado_jaccard=`../../GeneracionTexto/metricas/jaccard.py`
	echo $fileRNN "," $resultado_chi "," $resultado_chiLambda "," $resultado_total "," $resultado_jaccard >> ResultadosMetricas.txt
