...
public:
  ...
  void compress(int c) {
    ...
    /*for(int u=0; u<256; u++){
		std::cout << "VERIFICADOR "<<'\n' <<'\n' << '\n';
		std::cout <<'\n' << u<< ": " << unVerificador.verificar(c,8);
	}
	*/
    if (level==0)
      putc(c, archive);
...
//        printf("%s already exists\n", archiveName.c_str()), quit();
    ...
    if (mode==COMPRESS) {
		...
		generado = true;
		char emitidos[256];
		std::ofstream outfile("salidaPAQ");
		...
		U32 finalNum;
		for (int i = 0; i < 100048; i++){
			//srand(time(NULL)); // Seed the time
			finalNum = rand()%(en.x2-en.x1)+en.x1;
			...
			//	}
				...
				//if(unVerificador.verificar(resultado,count))
					en.code(0);
				//else{
				//	resultado++;
				//	resultado32++;
			...
			if (count==8){
				...
				emitidos[resultado] = 1;
				char charAImprimir = resultado;
				outfile.write(&charAImprimir, 1);
		...
		for(int i = 0; i < 1024; i++){
			...
			for (int i = 64; i < ventanaCotx.length(); i++){
				en.compress(ventanaCotx[i]);
				ventanaCotx[i-64] = ventanaCotx[i];
				ventanaCotxAux+=  ventanaCotx[i];
			}
			//en.compress('.');
			//ventanaCotxAux+= '.';
			//std::cout <<  '\n';
			//std::cout << "Ventana:"<< '\n';
			//std::cout << ventanaCotxAux <<'\n';
			//std::cout <<  '\n';
			//std::cout <<  "<<<<<<<<<<<<<<<< "<<'\n';
			ventanaCotx= ventanaCotxAux;
			ventanaCotxAux.clear();
		...
		*/
		...
		for(int u=0; u<256; u++){
			if(!unVerificador.verificar(u,8))
				std::cout <<'\n' <<(char) u<< ": VERIFICADOR  " << unVerificador.verificar(u,8) << " --- EMITIDO " << (int)emitidos[u] ;
		}
		printf("%ld -> %ld\n", total_size, en.size());
	}
