...
void Predictor::AddPPM() {
  Add(new PPM(15, manager_.bit_context_, 100, 80000000));//80000000
}

void Predictor::AddDMC() {
  Add(new DMC(0.02, 70000000));//70000000
}

void Predictor::AddByteRun() {
  unsigned long long max_size = 10000000; //10000000
  float delta = 200;
...
void Predictor::AddNonstationary() {
  unsigned long long max_size = 1000000; //1000000
  float delta = 500;
...
void Predictor::AddSparse() {
  ...
  for (const auto& params : model_params2) {
    ...
    const Context& context = manager_.AddContext(std::move(hash));
    Add(new Match(manager_.history_, context.context_, manager_.bit_context_,
        200, 0.5, 10000000, &(manager_.longest_match_))); //10000000
    Add(new ByteRun(context.context_, manager_.bit_context_, 100, 10000000));//10000000
  }
}
...
void Predictor::AddMatch() {
  float delta = 0.5;
  int limit = 200;
  unsigned long long max_size = 20000000; //20000000
