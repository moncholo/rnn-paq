...
void Predictor::update() {
  ...
  pr=pr+pr0+1>>1;
  //printf("Proxima predccion: %i \n", pr);
}
//~ 
//////////////////////////// Encoder ////////////////////////////

// An Encoder does arithmetic encoding.  Methods:
// Encoder(COMPRESS, f) creates  for compression to archive f, which
...
typedef enum {COMPRESS, DECOMPRESS, GENERATE} Mode;
class Encoder {
public:
  Predictor predictor;
  const Mode mode;       // Compress or decompress?
  FILE* archive;         // Compressed data file
  U32 x1, x2, xMid;            // Range, initially [0, 1), scaled by 2^32
  ...
  int code(int i=0) {
    ...
    while (((x1^x2)&0xff000000)==0) {  // pass equal leading bytes of range
      ...
      if (mode==DECOMPRESS) x=(x<<8)+(getc(archive)&255);  // EOF is OK
		...
		p=predictor.p();
		p+=p<2048;
		xMid=x1 + (x2-x1>>12)*p + ((x2-x1&0xfff)*p>>12);
    return y;
  }
  
  int codeModificado(int i = 0){
	int p=predictor.p();
    assert(p>=0 && p<4096);
    p+=p<2048;
    U32 xmid=x1 + (x2-x1>>12)*p + ((x2-x1&0xfff)*p>>12);
...
class Copiador{
	...
	int memusedcopy, maxmemcopy;
	clock_t start_timecopy;
	int poscopy;	
	Array<U8> bcopy;
	Array<int> bcopy2;
	...
	void serializarProgramChecker(int memused, int maxmem, clock_t start_time){
		memusedcopy=memused;
		maxmemcopy=maxmem;
		start_timecopy=start_time;
	}
	void serializarGlobales(){
		poscopy=pos;
		levelcopy=level;
		ycopy=y;
		c0copy=c0;
	...
	void serializarBuf(Array<U8> paramB){
		bcopy=paramB;
	}
	
	void serializarIntBuf(Array<int> paramB){
		bcopy2=paramB;
	}
...
//        printf("%s already exists\n", archiveName.c_str()), quit();
    ...
    if (mode==COMPRESS) {
      ...
      en.flush();
	/* A esta altura el archivo ya se comprimio,
	* con lo cual el modelo esta entrenado. La idea 
	* es generar un random y tirar uno a cero.
	*/
	printf("\n");
	int count = 0;
	int resultado = 0;
	...
	std::string nombreArchivo(fname[0]);
	
        std::string comando = "cr_checkpoint --file ";

        comando+=nombreArchivo+"_size:"+std::to_string(fsize[0])+"_level:"+std::to_string(level)+"_time:"+
		std::to_string(double(clock()-programChecker.start_time)/CLOCKS_PER_SEC)+"_maxMem:"+ std::to_string(programChecker.maxmem)
			 + "_PAQ_Checkpoint ";
	comando+="--term ";
	...
	//la siguiente matriz almacena para el 0 o 1  computadas por PAQ
	//primer columna tiene p0
	//segunda columna tiene p1
	//tercer columna tiene la prob del bit que genera
	double** matrix = new double*[3];
	matrix[0] = new double[cantidadDeCaracteres*8];
	matrix[1] = new double[cantidadDeCaracteres*8];
	matrix[2] = new double[cantidadDeCaracteres*8];
  ...
  //	myfile.open ("example.txt");
	...
	for (int i = 0; i < cantidadDeCaracteres*8; i++){
		U32 random = rand()%(en.x2-en.x1);		
		U32 finalNum = random+en.x1;
		if ((finalNum < en.x1) || (finalNum > en.x2)) { printf ("ERROR \n");}

		resultado = resultado<<1;
		double p1 = (double) en.p();		
		double p0 = 4096-p1;
...
//	myfile.close();
	...
	long double N = (long double)cantidadDeCaracteres*8;
	//aqui calculo la perplejidad
	for (int i = 0; i < N; i++){
		long double log2Probabilidad = log2 ((long double)matrix[2][i]);
		entropia = entropia - log2Probabilidad / N;		
	}

	long double perplexity = pow (2,entropia);
//	printf("Entropia: %Le \n", entropia);
//	printf("Perplexity: %Le \n", perplexity);
