...
void Compress(unsigned long long input_bytes, std::ifstream* is,
  ...
  printf("Serializar Compresor");
  int cantidadDeCaracteres;
  int status=scanf ("%i",&cantidadDeCaracteres);
  
  if (cantidadDeCaracteres != 0){
	  std::string input_path = "input.txt";	
	  std::ifstream concatenado(input_path, std::ios::in | std::ios::binary);
	  FILE* data_in = fopen(input_path.c_str(), "rb");

	  fseek(data_in, 0L, SEEK_END);

	  unsigned long long tamanio = ftell(data_in);
	  printf("Tamanio: %lu", tamanio);
	  for (unsigned long long pos = 0; pos < tamanio; ++pos) {
	    char c = (&concatenado)->get();
	    for (int j = 7; j >= 0; --j) {
	      e.Encode((c>>j)&1);
	    }
	    if (pos % percent == 0) {
