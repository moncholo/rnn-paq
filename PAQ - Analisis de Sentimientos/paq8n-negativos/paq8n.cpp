...
//        printf("%s already exists\n", archiveName.c_str()), quit();
    ...
    if (mode==COMPRESS) {
//aca se comprimen todos hasta el ultimo archivo sin incluirlo
//ese ultimo archivo es review.txt
      for (int i=0; i<files-1; ++i)
	compress(fname[i], fsize[i], en);

     //Ahora se congela el proceso
     printf("Congelando proceso \n");
     int numeroPID=getpid();		
     std::string s = std::to_string(numeroPID);
     std::string comando = "cr_checkpoint --file CheckpointNegativos --term ";
     comando+=s;
